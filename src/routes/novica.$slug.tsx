...
interface NovicaResponse {
  data: DrupalNovica;
  included?: DrupalIncluded[];
  nextNovica?: { title: string; path: { alias: string | null } } | null;
  prevNovica?: { title: string; path: { alias: string | null } } | null;
  articleId: string;
  ...
  const res = await fetch(`${DRUPAL_BASE}/jsonapi/comment/comment`, {
    ...
    body: JSON.stringify({
      data: {
        ...
        relationships: {
          entity_id: { data: { type: "node--novica", id: articleId } },
        },
      },
    }),
  });
...
const getNovica = createServerFn({ method: "GET" }).handler(
  async (ctx): Promise<NovicaResponse> => {
    ...
    if (isUuid) {
      ...
      const routerRes = await fetch(
        `${DRUPAL_BASE}/router/translate-path?path=/novica/${slug}`,
      );
      if (!routerRes.ok) throw new Error("Novica not found");
    ...
    const res = await fetch(
      `${DRUPAL_BASE}/jsonapi/node/novica/${uuid}?include=field_field_naslovna_slika,field_avtor,field_galerija,field_oznake`,
    );
    if (!res.ok) throw new Error("Failed to fetch novica");
    ...
    if (nextRes.ok) {
      ...
      if (nextJson.data.length > 0) nextNovica = nextJson.data[0].attributes;
    }
    ...
    if (prevRes.ok) {
      ...
      if (prevJson.data.length > 0) prevNovica = prevJson.data[0].attributes;
    }
...
export const Route = createFileRoute("/novica/$slug")({
  head: ({ loaderData }) => {
    const d = loaderData as NovicaResponse | undefined;
    return {
      meta: [
        { title: `Zgodbe o Pomurju — ${d?.data?.attributes?.title ?? ""}` },
      ],
    };
  },
  component: NovicaPage,
  loader: ({ params }) => getNovica({ data: params.slug as never }),
});
...
function NovicaPage() {
  ...
  } = Route.useLoaderData() as NovicaResponse;
  ...
  const galleryIds = novica.relationships.field_galerija?.data ?? [];
  ...
  const tagRefs = novica.relationships.field_oznake?.data ?? [];
  const tags = tagRefs
    .map((ref: { id: string }) =>
      included?.find(
  ...
  const imageId = novica.relationships.field_field_naslovna_slika.data?.id;
  ...
  const authorId = novica.relationships.field_avtor.data?.id;
  const authorName = included?.find(
    (f: DrupalIncluded) => f.id === authorId && f.type === "node--author",
  )?.attributes.title;

  const date = new Date(novica.attributes.created).toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
  });
    ...
    <main className="fade-in-up">
      ...
      <ScrollReveal>
        <div className="max-w-3xl mx-auto px-6 pb-16">
          {novica.attributes.field_telo[0] && (
            <div
              className="prose prose-lg max-w-none break-words"
              dangerouslySetInnerHTML={{
                __html: novica.attributes.field_telo[0].processed.replace(
                  /src="\/sites\//g,
      ...
      <ScrollReveal delay={200}>
        {(prevNovica?.path?.alias || nextNovica?.path?.alias) && (
          <div className="border-t border-gray-200 max-w-5xl mx-auto px-6 mt-4 mb-16">
            <div className="flex items-center justify-between py-10">
              {nextNovica?.path?.alias ? (
                <a
                  href={nextNovica.path.alias}
              ...
              {prevNovica?.path?.alias ? (
                <a
                  href={prevNovica.path.alias}
